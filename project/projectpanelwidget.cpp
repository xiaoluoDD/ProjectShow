...
#include "projectstatusutils.h"
#include "datepickerutils.h"
#include "tableheaderfilter.h"
    ...
    : QWidget(parent)
{
    ...
    auto *title = new QLabel(QStringLiteral("项目面板"), this);
    ...
    m_statusLabel = new QLabel(
        QStringLiteral("支持项目增删改；双击日期列可快速修改；双击「项目任务」列管理子任务，双击其他列查看项目详情。"
                       "点击「年度 / 工番号 / 项目负责人 / 项目状态」表头可筛选。"),
        this);
    ...
    m_sendBtn = new QPushButton(QStringLiteral("向项目负责人发送提醒"), this);
...
void ProjectPanelWidget::setupProjectTable()
{
    ...
    m_projectTable->setHorizontalHeaderLabels({
        ...
        QStringLiteral("项目启动日期"), QStringLiteral("实际完结日期"), QStringLiteral("项目任务"),
        QStringLiteral("项目成员"),
    });
    ...
    m_projectTable->setItemDelegateForColumn(ColStatus, ProjectStatusUtils::createStatusItemDelegate(m_projectTable));
    m_projectTable->viewport()->installEventFilter(this);
}

bool ProjectPanelWidget::eventFilter(QObject *watched, QEvent *event)
{
    if (watched == m_projectTable->viewport() && event->type() == QEvent::MouseButtonPress) {
        const auto *mouseEvent = static_cast<QMouseEvent *>(event);
        if (!m_projectTable->indexAt(mouseEvent->pos()).isValid())
            m_projectTable->clearSelection();
    }
    return QWidget::eventFilter(watched, event);
}

void ProjectPanelWidget::setupHeaderFilter()
{
    m_headerFilter = new TableHeaderFilter(m_projectTable, this);
    m_headerFilter->setBaseLabels({
        QStringLiteral("年度"), QStringLiteral("工番号"), QStringLiteral("项目名称"),
        QStringLiteral("项目负责人"), QStringLiteral("项目状态"),
        QStringLiteral("项目启动日期"), QStringLiteral("实际完结日期"), QStringLiteral("项目任务"),
        QStringLiteral("项目成员"),
    });
    m_headerFilter->addFilterColumn(ColYear, [](const QJsonObject &p) {
        return p.value(QStringLiteral("year")).toString();
    });
    m_headerFilter->addFilterColumn(ColWorkNo, [](const QJsonObject &p) {
        return p.value(QStringLiteral("work_no")).toString();
    });
    m_headerFilter->addFilterColumn(ColManager, [](const QJsonObject &p) {
        const QString name = p.value(QStringLiteral("manager_name")).toString().trimmed();
        if (!name.isEmpty())
            return name;
        return p.value(QStringLiteral("manager_userid")).toString();
    });
    m_headerFilter->addFilterColumn(ColStatus, [](const QJsonObject &p) {
        const QString status = p.value(QStringLiteral("status")).toString().trimmed();
        if (!status.isEmpty())
            return status;
        return ProjectStatusUtils::effectiveProjectStatus(p);
    });
    connect(m_headerFilter, &TableHeaderFilter::filteredDataChanged, this,
            &ProjectPanelWidget::applyFilteredProjects);
}
...
void ProjectPanelWidget::applyFilteredProjects(const QJsonArray &projects)
{
    ...
    for (int r = 0; r < projects.size(); ++r) {
        ...
        const QString startDate = ProjectStatusUtils::displayStartDate(p);
        const QString endDate = ProjectStatusUtils::displayEndDate(p);
        QString displayStatus = p.value(QStringLiteral("status")).toString().trimmed();
        if (displayStatus.isEmpty())
            displayStatus = ProjectStatusUtils::effectiveProjectStatus(p);
        setCell(ColStatus, displayStatus);
        ProjectStatusUtils::applyStatusCellStyle(m_projectTable->item(r, ColStatus), displayStatus);

        setCell(ColStartDate, startDate.isEmpty() ? QStringLiteral("—") : startDate);
        setCell(ColEndDate, endDate.isEmpty() ? QStringLiteral("—") : endDate);
        setCell(ColTasks, formatTasksSummary(p));
        setCell(ColMembers, formatMembersSummary(p));
    }
    adjustProjectTableColumns();

    const int total = m_headerFilter ? m_headerFilter->sourceCount() : projects.size();
    if (total > 0 && projects.size() != total) {
        m_statusLabel->setText(
            QStringLiteral("显示 %1 / %2 个项目").arg(projects.size()).arg(total));
    } else {
        m_statusLabel->setText(QStringLiteral("共 %1 个项目").arg(projects.size()));
    }
}

QString ProjectPanelWidget::formatTasksSummary(const QJsonObject &project)
{
    const QString summary = project.value(QStringLiteral("task_summary")).toString().trimmed();
    if (!summary.isEmpty())
        return summary;
...
QJsonObject ProjectPanelWidget::projectAtRow(int row) const
{
...
const char *ProjectPanelWidget::dateFieldForColumn(int column)
{
    switch (column) {
    case ColStartDate:
        return "start_date";
...
void ProjectPanelWidget::pickAndSaveProjectDate(int row, int column)
{
    const char *field = dateFieldForColumn(column);
    if (!field)
        return;

    QJsonObject project = projectAtRow(row);
    if (project.isEmpty())
        return;

    const QString fieldKey = QString::fromUtf8(field);
    QString picked;
    const QString title = m_projectTable->horizontalHeaderItem(column)->text();
    if (!DatePickerUtils::pickDateDialog(this, title, project.value(fieldKey).toString(), &picked))
        return;

    if (fieldKey == QStringLiteral("start_date") && picked.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("提示"), QStringLiteral("项目启动日期不能为空"));
        return;
    }

    if (fieldKey == QStringLiteral("end_date") && !picked.isEmpty()) {
        const int subCount = project.value(QStringLiteral("subtask_count")).toInt();
        const bool allDone = project.value(QStringLiteral("subtask_all_completed")).toBool();
    ...
    project.insert(fieldKey, picked);
    project.remove(QStringLiteral("members"));
    const QByteArray body = QJsonDocument(project).toJson(QJsonDocument::Compact);
...
{
    return projectAtRow(m_projectTable->currentRow());
}
...
void ProjectPanelWidget::updateNotifyTargetLabel()
{
    ...
    const QString mgrName = p.value(QStringLiteral("manager_name")).toString();
    if (mgrId.isEmpty()) {
        m_notifyTargetLabel->setText(
            QStringLiteral("提醒对象：项目「%1」尚未配置负责人，请先编辑项目")
                .arg(projectName));
        return;
    }

    const QString mgrLabel = mgrName.isEmpty() ? mgrId : QStringLiteral("%1（%2）").arg(mgrName, mgrId);
    m_notifyTargetLabel->setText(
        QStringLiteral("提醒对象：项目「%1」负责人 %2")
            .arg(projectName, mgrLabel));
}
...
bool ProjectPanelWidget::checkServerUrl(QString *baseOut)
{
    ...
    if (base.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("提示"), QStringLiteral("请在「调试 → 设置」中配置后端地址"));
        return false;
    }
    QSettings settings;
...
void ProjectPanelWidget::showProjectDialog(const QJsonObject &existing)
{
    ...
    if (dlg.isEditMode() && !payload.value(QStringLiteral("end_date")).toString().trimmed().isEmpty()) {
        const int subCount = existing.value(QStringLiteral("subtask_count")).toInt();
        const bool allDone = existing.value(QStringLiteral("subtask_all_completed")).toBool();
        if (subCount > 0 && !allDone) {
            QMessageBox::warning(
                this, QStringLiteral("子任务未完成"),
                QStringLiteral("存在未完成的子任务，不能填写实际完结日期。\n请先将全部子任务标记为已完结。"));
...
void ProjectPanelWidget::onProjectDoubleClicked(int row, int column)
{
    ...
    const QJsonObject p = projectAtRow(row);
    if (p.isEmpty())
        return;

    if (dateFieldForColumn(column)) {
        pickAndSaveProjectDate(row, column);
        return;
    }
...
void ProjectPanelWidget::onReplyFinished(QNetworkReply *reply)
{
    ...
    if (kind == ProjectPanelRequests::Kind::LoadProjects) {
        ...
        m_headerFilter->setSourceData(obj.value(QStringLiteral("projects")).toArray());
...
void ProjectPanelWidget::navigateToProject(int projectId, bool openSubtasks)
{
    if (projectId <= 0)
        return;

    m_pendingNavigateProjectId = projectId;
    m_pendingOpenSubtasks = openSubtasks;

    for (int row = 0; row < m_projectTable->rowCount(); ++row) {
        const QJsonObject project = projectAtRow(row);
        if (project.value(QStringLiteral("id")).toInt() == projectId) {
            m_projectTable->selectRow(row);
            m_projectTable->scrollToItem(m_projectTable->item(row, 0));
            completePendingNavigation();
            return;
        }
    }
...
void ProjectPanelWidget::completePendingNavigation()
{
    if (m_pendingNavigateProjectId <= 0)
        return;

    QJsonObject project;
    for (int row = 0; row < m_projectTable->rowCount(); ++row) {
        const QJsonObject candidate = projectAtRow(row);
        if (candidate.value(QStringLiteral("id")).toInt() == m_pendingNavigateProjectId) {
            project = candidate;
            m_projectTable->selectRow(row);
            if (m_projectTable->item(row, 0))
                m_projectTable->scrollToItem(m_projectTable->item(row, 0));
            break;
        }
    }

    const bool openSubtasks = m_pendingOpenSubtasks;
    m_pendingNavigateProjectId = -1;
    m_pendingOpenSubtasks = false;

    if (project.isEmpty())
        return;

    if (openSubtasks)
        showSubTaskDialog(project);
    else
